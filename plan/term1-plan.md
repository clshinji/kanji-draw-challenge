...
- **構成**: AWS サーバレス（DynamoDB無料枠 + Cognito認証）
...
| 順序 | タスク | 理由 |
|------|--------|------|
| 1 | Task 0-1（姉バグ修正） | 最優先のバグ修正 |
| 2 | Task 1-1（ローディング） | canvas-drawing.js を続けて改修 |
| 3 | Task 2-1（カテゴリアンロック演出） | フロントのみで完結 |
| 4 | Task 1-2（クラウド同期） | AWS構築を含む最大タスク |
